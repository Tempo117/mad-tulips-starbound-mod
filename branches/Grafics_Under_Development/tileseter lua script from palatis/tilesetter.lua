...
function onInteraction(args)
	...
	local cur_Neigh_Pos = {};
	local cur_F_mat = nil;
	local f_tileset_mat_group = nil -- nil,human,glitch ...
	...
	for layer_nr=1,2,1 do
		...
		for X=self.BB[1],self.BB[3],1 do
			for Y=self.BB[2],self.BB[4],1 do
				...
				f_tileset_mat_group = get_tileset_mat_group(cur_F_mat)
				...
				if(f_tileset_mat_type ~= nil) then
					...
					for X_rel=-1,1,1 do
						for Y_rel=-1,1,1 do
							...
							neigh_tileset_mat_group = get_tileset_mat_group(cur_neigh_mat)
							neigh_tileset_mat_type = get_tileset_mat_type(cur_neigh_mat)
							if (f_tileset_mat_group == neigh_tileset_mat_group) then
								place_block_here = true
								if     ((X_rel ==  0) and (Y_rel ==  1)) then self.neighbours[1] = true -- top
								elseif ((X_rel ==  1) and (Y_rel ==  1)) then self.neighbours[2] = true -- top right
								elseif ((X_rel ==  1) and (Y_rel ==  0)) then self.neighbours[3] = true -- right
								elseif ((X_rel ==  1) and (Y_rel == -1)) then self.neighbours[4] = true -- bottom right
								elseif ((X_rel ==  0) and (Y_rel == -1)) then self.neighbours[5] = true -- bottom
								elseif ((X_rel == -1) and (Y_rel == -1)) then self.neighbours[6] = true -- bottom left
								elseif ((X_rel == -1) and (Y_rel ==  0)) then self.neighbours[7] = true -- left
								elseif ((X_rel == -1) and (Y_rel ==  1)) then self.neighbours[8] = true -- top left
								end 
							end
						end
					end
					...
					-- **    ***   ***    **
					-- *x*   *x*   *x*   *x* , thick corner
					-- *** , **  ,  ** , ***
					if     (self.neighbours[8] and self.neighbours[1] and
							self.neighbours[7] and 						  self.neighbours[3] and 
							self.neighbours[6] and self.neighbours[5] and self.neighbours[4]) then own_mat_to_be_set = "tr"
					elseif (self.neighbours[8] and self.neighbours[1] and self.neighbours[2] and 
							self.neighbours[7] and 						  self.neighbours[3] and 
							self.neighbours[6] and self.neighbours[5]                       ) then own_mat_to_be_set = "br"
					elseif (self.neighbours[8] and self.neighbours[1] and self.neighbours[2] and 
							self.neighbours[7] and 						  self.neighbours[3] and 
							                       self.neighbours[5] and self.neighbours[4]) then own_mat_to_be_set = "bl"
					elseif (                       self.neighbours[1] and self.neighbours[2] and 
							self.neighbours[7] and 						  self.neighbours[3] and 
							self.neighbours[6] and self.neighbours[5] and self.neighbours[4]) then own_mat_to_be_set = "tl"
					-- ***   **           **
					-- *x*   *x    *x*    x* , thick straight
					--     , **  , *** ,  **
					elseif (self.neighbours[8] and self.neighbours[1] and self.neighbours[2] and
							self.neighbours[7] and                        self.neighbours[3]) then own_mat_to_be_set = "lr"
					elseif (self.neighbours[7] and                        self.neighbours[3] and
							self.neighbours[6] and self.neighbours[5] and self.neighbours[4]) then own_mat_to_be_set = "lr"
					elseif (self.neighbours[8] and self.neighbours[1] and
							self.neighbours[7] and
							self.neighbours[6] and self.neighbours[5]) then own_mat_to_be_set = "tb"
					elseif (self.neighbours[1] and self.neighbours[2] and
							                       self.neighbours[3] and
							self.neighbours[5] and self.neighbours[4]) then own_mat_to_be_set = "tb"
					-- +
					elseif (self.neighbours[1] and self.neighbours[3] and self.neighbours[5] and self.neighbours[7]) then own_mat_to_be_set = "trbl"
					-- T
...
function get_tileset_mat_group(cur_F_mat)
	-- returns the mat group of the mat (human,glitch ..) or nil if unknown
	if cur_F_mat == nil then return nil end
	if cur_F_mat == false then return nil end
	
	if string.find(cur_F_mat, "madtulip_apex3_") then return "apex" end
	if string.find(cur_F_mat, "madtulip_apex2_") then return "apex" end
	if string.find(cur_F_mat, "madtulip_apex_") then return "apex" end
	if string.find(cur_F_mat, "madtulip_avian3_") then return "avian" end
	if string.find(cur_F_mat, "madtulip_avian2_") then return "avian" end
	if string.find(cur_F_mat, "madtulip_avian_") then return "avian" end
	if string.find(cur_F_mat, "madtulip_floran3_") then return "floran" end
	if string.find(cur_F_mat, "madtulip_floran2_") then return "floran" end
	if string.find(cur_F_mat, "madtulip_floran_") then return "floran" end
	if string.find(cur_F_mat, "madtulip_glitch3_") then return "glitch" end
	if string.find(cur_F_mat, "madtulip_glitch2_") then return "glitch" end
	if string.find(cur_F_mat, "madtulip_glitch_") then return "glitch" end
	if string.find(cur_F_mat, "madtulip_human3_") then return "human" end
	if string.find(cur_F_mat, "madtulip_human2_") then return "human" end
	if string.find(cur_F_mat, "madtulip_human_") then return "human" end
	if string.find(cur_F_mat, "madtulip_hylotl3_") then return "hylotl" end
	if string.find(cur_F_mat, "madtulip_hylotl2_") then return "hylotl" end
	if string.find(cur_F_mat, "madtulip_hylotl_") then return "hylotl" end
	if string.find(cur_F_mat, "madtulip_novakid3_") then return "novakid" end
	if string.find(cur_F_mat, "madtulip_novakid2_") then return "novakid" end
